...
def predict_rating(user, item, k=5):
    ...
    if user in user_means:
        user_deviation = user_means[user] - global_baseline
    else:
        user_deviation = 0
    if item in item_means:
        item_deviation = item_means[item] - global_baseline
    else:
        item_deviation = 0
    
    baseline_prediction = global_baseline + user_deviation + item_deviation

    baseline_prediction = max(0, baseline_prediction)
    baseline_prediction = min(baseline_prediction, 5)
